const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*𝐅𝚰 𝚩𝚯𝚻 𝐔𝐆 ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "𝐅𝚰 𝚩𝚯𝚻 𝐔𝐆 ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("𝐅𝚰 𝚩𝚯𝚻 𝐔𝐆• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("𝐅𝚰 𝚩𝚯𝚻 𝐔𝐆 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*𝐅𝚰 𝚩𝚯𝚻 𝐔𝐆 • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
